(function() {
    ...
    callWithJQuery(function($) {
      ...
      $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
          var areaToAddVariable; // use after refresh function assignment
        ...
        try {
          ...
          rendererControl = $("<div>").addClass("pvtUiCell form-group col-md-6");
          rendererControl.append($("#pivottable_components #table_type")); // Tablo Çeşidi
          renderer = $("<select>").addClass('pvtRenderer form-control').appendTo(rendererControl).bind("change", function() {
            return refresh();
          });
          ref = opts.renderers;
          for (x in ref) {
          ...
          unused = $("<div>", {"id": "unused"}).addClass('pvtAxisContainer pvtUnused pvtUiCell d-flex flex-wrap');
          ...
          fn1 = function(attr) {
            ...
            if (values.length > opts.menuLimit) {
              ...
              if (values.length > 5) {
                ...
                $("<input>", {
                  ...
                  "class": "pvtSearch form-control mx-auto"
                }).bind("keyup", function() {
              ...
              for (n = 0, len2 = ref1.length; n < len2; n++) {
                ...
                $("<input>").attr("type", "checkbox").addClass('pvtFilter').attr("checked", !filterItemExcluded).attr("data-header", attr).attr("data-subheader", value).data("filter", [attr, value]).appendTo(filterItem).bind("change", function() {
            ...
            closeFilterBox = function() {
              ...
              if (!$("#modalForVariable .modal-header").is(":visible")){
                $("#modalForVariable").modal("hide");
              }

              return valueList.hide();
            };
            finalButtons = $("<p>", {"class": "clearfix"}).appendTo(valueList);
            if (values.length <= opts.menuLimit) {
              $("<button>", {
                type: "button",
            ...
            triangleLink = $("<span>").addClass('pvtTriangle').html(" &#x25BE;").bind("click", function(e) {
              ...
              return valueList.show();
            });
            attrElem = $("<li>").addClass("axis_" + i + " border rounded-lg border-dark")
                                .append($("<span>").addClass('pvtAttr').append($("<span>", {"class": "attrName", "data-name": attr}).text(attr)).data("attrName", attr)
                                                                       .append($("<span>", {"class": "attrIcon"}).append(triangleLink)));
          ...
          uiTable.append($("<div>", {"id": "configContainer", "class": "container"}));

          tr1 = $("<div>", {"id": "tableOption"}).appendTo(uiTable.find("#configContainer"));
          tr1 = $("<div>", {"class": "row"}).appendTo(tr1); 
          ...
          rowOrderArrow = $("#pivottable_components #sort_rows").addClass("pvtRowOrder").data("order", opts.rowOrder).html(ordering[opts.rowOrder].rowSymbol).bind("click", function() {
            $(this).data("order", ordering[$(this).data("order")].next);
            $(this).html(ordering[$(this).data("order")].rowSymbol);
            return refresh();
          });
          colOrderArrow = $("#pivottable_components #sort_columns").addClass("pvtColOrder").data("order", opts.colOrder).html(ordering[opts.colOrder].colSymbol).bind("click", function() {
            $(this).data("order", ordering[$(this).data("order")].next);
            $(this).html(ordering[$(this).data("order")].colSymbol);
            return refresh();
          });
          
          $("<div>").addClass('pvtVals pvtUiCell form-group col-md-6').append($("#pivottable_components #table_operation")).append(aggregator).appendTo(tr1); // Tablo İşlemi
          
          tr2 = $("<div>", {"id": "tableArea", "class": "px-5"}).appendTo(uiTable);

          tr2.append($("<div>", {"class": "row"})).append($("<div>", {"class": "row"}));

          tr2.find("div.row:nth-child(1)")
              .append($("<div>", {"id": "features", "class": "d-flex justify-content-end align-items-center"}).css("width", "235px")
              .append($("#reset_button"))
              .append($("#pivottable_components #fullscreen_icon"))
              .append(rowOrderArrow)
              .append(colOrderArrow));

          tr2.find("div.row:nth-child(1)")
            .append($("<div>")
                .addClass('pvtAxisContainer pvtHorizList pvtCols pvtUiCell border d-flex')
                .append($("#pivottable_components #add_column")));
        
          tr2.find("div.row:nth-child(2)")
            .append($("<div>")
                .addClass('pvtAxisContainer pvtRows pvtUiCell border')
                .append($("#pivottable_components #add_row")));

          
          pivotTable = $("<div>").attr("valign", "top").addClass('pvtRendererArea responsive-table').appendTo(tr2.find("div.row:nth-child(2)"));

        //   if (opts.unusedAttrsVertical === true || unusedAttrsVerticalAutoOverride) {
        //     uiTable.prepend(rendererControl);
        //     uiTable.prepend(unused);
        //   } else {
        //     uiTable.prepend($("<div>").append(rendererControl).append(unused));
        //   }
            tr1.prepend(rendererControl);
            
            $("#modalForVariable .modal-body").append(unused);
            ...
            return function() {
              ...
              if (numInputsToProcess !== 0) {
                ...
                for (x = t = 0, ref5 = numInputsToProcess; 0 <= ref5 ? t < ref5 : t > ref5; x = 0 <= ref5 ? ++t : --t) {
                  ...
                  newDropDown = $("<div>", {"class": "form-group"}).append(newDropdown);
              ...
              exclusions = {};
              $("#modalForVariable").find('input.pvtFilter').not(':checked').each(function() {
                  var filter;
              ...
              inclusions = {};
              $("#modalForVariable").find('input.pvtFilter:checked').each(function() {
                  var filter;
                  filter = $(this).data("filter");
                  if (exclusions[filter[0]] != null) {
          ...
          $("#modalForVariable").on("show.bs.modal", function(event) {
              areaToAddVariable = $(event.relatedTarget).data("whatever");
          });

          $("#modalForVariable .modal-body").on("click", "li", function(event) {
              if ($(event.target).hasClass("pvtTriangle")) return false;
              
              $(this).find(".attrIcon").append($("<button>", {"class": "close", "type": "button"}).html("&times;"));

              $(this).appendTo(areaToAddVariable);
              $("#modalForVariable").modal("hide");
              refresh();
          });

          $(".pvtCols, .pvtRows").on("click", ".close", function(){
            $(this).parents("li").appendTo(unused);
            $(this).remove();
            refresh();
        });

          $(".pvtCols, .pvtRows").on("click", ".pvtTriangle", function(){
              $("#modalForVariable").addClass("d-flex justify-content-center")
                                    .find(".modal-dialog").removeClass("modal-dialog-centered")
                                    .find(".modal-header, .modal-footer, .modal-content li").addClass("d-none")
                                    .parents(".modal-content").css("background-color", "transparent") 
                                    .parents("#modalForVariable").modal("show");
          });

          

          $("#modalForVariable").on("hide.bs.modal", function () {
              $("#modalForVariable").removeClass("d-flex justify-content-center")
                                    .find(".modal-dialog").addClass("modal-dialog-centered")
                                    .find(".modal-header, .modal-footer, .modal-content li").removeClass("d-none")
                                    .parents(".modal-content").css("background-color", "#fff")
                                    .find(".pvtFilterBox").hide();
              refresh();
          });
