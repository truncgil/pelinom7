					...
					<table class="table table-striped table-hover table-bordered table-vcenter">
						<thead>
							<tr>
								...
								<th>{{__("Ayar Başlığı")}}</th>
								<th>{{__("Ayar İçeriği")}}</th>
								<th>{{__("Ayar Türü")}}</th>
						...
						<tbody> @foreach($alt AS $a) <tr class="">
								<th class="text-center d-none cover" scope="row"> @if($a->cover!='') <a
								...
								<td>
                                    <input type="text" name="alt_type" value="{{$a->alt_type}}" table="contents"
										id="{{$a->id}}" class="alt_type{{$a->id}} form-control edit" />
								 </td>
